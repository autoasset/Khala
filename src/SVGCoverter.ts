...
class SVGCoverter {
    ...
    async vectordrawable(file: string, name: string, data: Buffer) {
        const options = {
        ...
        FilePath.write(path, xml)
    }

    async run(file: string, name: string) {
        const result = optimize(fs.readFileSync(file), {
            path: file,
            plugins: [
                "removeDoctype",
                "removeXMLProcInst",
        ...
        FilePath.write(path, result.data)
        await this.pdf(path, name, data)
        await this.vectordrawable(path, name, data)
    ...
    async pdf(file: string, name: string, data: Buffer) {
